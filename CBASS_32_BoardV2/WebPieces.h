...
<div id="links" class="wrapper flex fittwowide" style="height:fit-content;">
<b>Any Time</b><br/>
<ul>
...
<div id="fullScreenClicker" class="clickme" style="position:absolute; padding: 0px; top:0px; left:0px; width:38px; height:38px; background-color:#F0F0F0; z-index:1001" >
			<img src="/fullScreenIcon.svg" width="38" height="38">
</div>
<!-- <div id="bbb"  class="wrapper flex fittwowide"  style="height:30px;">Nothing yet.</div> -->
<!-- Links before script so it is defined and we can manipulate it. -->
~LINKLIST~

<script>
	TESTER = document.getElementById('historyPlot');
	CLICKER = document.getElementById('fullScreenClicker');
	STATS = document.getElementById('statusTable');
	LINKS = document.getElementById('links');
	CLICKER.addEventListener('click', function() {
					console.log("Clicked no JQ");
					TESTER.classList.toggle("expanded");
					TESTER.classList.toggle("fittwowide");
					STATS.classList.toggle("hidediv");
					LINKS.classList.toggle("hidediv");
          STATS.classList.toggle("fittwowide");
					LINKS.classList.toggle("fittwowide");
  ...
  // Just pasting ° didn't work 176 gives the symbol and 8451 gives it with the C.
  // nonnegative ensures that the graph doesn't go all the way to -127 in the case of a bad temperature value.
  yaxis: {title: 'T, '+ String.fromCodePoint(176) + 'C', rangemode: "nonnegative"},  
  ...
  async function getPoints(TESTER) {
    ...
    let delta = 0.0;
    for (let i=1; i <= ~NT~; i++) {
      document.getElementById('temp' + i).innerHTML=pt.actual[i-1].toFixed(1); //jjj.tempList[i-1];
      ...
      document.getElementById('set' + i).innerHTML=pt.target[i-1].toFixed(1);
    }

    const now = new Date();
    const timeparts = pt.datetime.split('T');
    //document.getElementById('cbassTime').innerText=`CBASS time: ${pt.datetime}`
    document.getElementById('cbassTime').innerText=`CBASS time: ${timeparts[0]} ${timeparts[1]}`
